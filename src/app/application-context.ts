...
export class ApplicationContext {
  ...
  //-------------------------------------------
  public IsPermissionForRequest(
    reqType: RequestDocumentType,
    mode: PermissionMode
  ): boolean {
  ...
  public get isLogin(): boolean {
    ...
    return (
      this.currentUser.trader_id > 0 &&
      this.currentUser.user_name.trim().length > 0
    );
  }
  //-------------------------------------------
  ...
  public gotoNewDraft(reqType: RequestDocumentType) {
    ...
    switch (reqType) {
      ...
      case RequestDocumentType.SendSample:
        url = routeDef.buildPathWithRoot(
          routeDef.pageNew,
          routeDef.pageNew_SendSample
        );
        break;
      case RequestDocumentType.Substitute:
        url = routeDef.buildPathWithRoot(
          routeDef.pageNew,
          routeDef.pageNew_Substiture
        );
        break;
  ...
  public gotoRenewPageWithReferenceToLicense(license_id: number) {
    ...
    let url = routeDef.buildPathWithRoot(
      routeDef.pageNew,
      routeDef.pageNew_Renewal,
      "license",
      license_id
  ...
  public ShowWaitingDialog(
    message: string = "",
    header: string = "",
    withLoadingImage: boolean = true
  ) {
    ...
    if (docExpire.length > 0) {
      ...
      const errorList = docExpire.map((doc: TraderDocumentFromELicensing) => {
        const message =
          "+ " +
          doc.document_name +
          "หมดอายุ [วันที่หมดอายุ " +
          fnToDateString(doc.expiry_date) +
          "]";
        return message;
      });
      ...
      this.errorDialog.openDialogWithMessage(
        "ไม่สามารถใช้งานในส่วนของคำขอใบอนุญาตได้ เนื่องจาก",
        errorList
      );
    }
  }
